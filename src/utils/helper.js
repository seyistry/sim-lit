...
export const nextAppointmentDate = (lastPickup, ARVRefill) => {
  return moment(lastPickup, "DD/MM/YYYY", true)
    .add(parseInt(ARVRefill), "day")
    .format("DD/MM/YYYY");
};
...
export const LTFUPatients = (store) => {
  const info = store.filter(
    (list) => list.CurrentARTStatus_Pharmacy === "LTFU" && list.Outcomes === ""
  );
  return info;
};
...
export const missedAppointment = (
  ...
  const info = store.filter((list) => {
    const nextAppointmentDate = moment(
      list.Pharmacy_LastPickupdate,
      "DD/MM/YYYY",
      true
    ).add(parseInt(list.DaysOfARVRefill), "day");

    const ReffDateStart = moment(date, "DD/MM/YYYY", true).subtract(28, "day");
